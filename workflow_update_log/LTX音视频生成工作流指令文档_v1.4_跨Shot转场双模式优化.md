...
> v1.4 更新重点：在 v1.2 / v1.3 的基础上，正式加入 **跨 Shot 转场桥接（Bridge Shot）机制**，并明确区分两种不同的拼接模式：
> 1. **Direct Concatenation Mode（直接拼接模式）**
> 2. **Trim-and-Overlap Mode（裁剪余量模式）**
>
> 两种模式都可以写入代码分支中，由程序根据工作流设置、后期能力和目标总时长自动选择。
...
9. 当单个 shot 受显存限制不能容纳所有关键帧时，如何通过 **bridge shot** 平滑跨段衔接。
...
Step 6  如果单个 shot 长度受限，判断是否需要使用 bridge shot
Step 7  判断使用 Direct Concatenation Mode 还是 Trim-and-Overlap Mode
Step 8  为每个 shot 设置 images、image_idxs、image_strengths、video_seconds
Step 9  按 LTX prompt 规范生成英文 prompt 与 negative_prompt
Step 10 输出 LTX Shot Plan 表和逐 shot 参数代码块
Step 11 给出拼接建议和常见失败修正策略
```
...
### 7.4 什么时候必须使用 bridge shot

以下情况建议强制插入 bridge shot：

1. 单个 shot 最多只能 10–12 秒，无法把全部关键帧塞入一个长 shot；
2. shot1 与 shot2 各自内部都流畅，但直接拼接时 frame03 → frame04 边界突兀；
3. shot1 的最后一个参考帧与 shot2 的第一个参考帧属于不同机位、不同景别、不同阶段；
4. 需要用光效、遮挡、动作桥、轨迹桥等方式显式过渡；
5. 当前项目不允许随机硬切。
...
Bridge Shot 1-2: frame03 -> frame04
...
所以需要专门设计 **cross-shot handoff**。

### 12.2 Bridge Shot 定义

bridge shot 是插入在两个主 shot 之间、专门用来完成跨段过渡的短 shot。

其目标不是创造新剧情，而是：
- 吸收 shot1 结尾动量；
- 过渡到 shot2 开头状态；
- 通过光效、遮挡、动作桥、镜头路径、音频桥等方式消除硬切感。

### 12.3 Bridge Shot 最常见结构

```text
bridge shot = [last anchor of previous shot] -> [first anchor of next shot]
```
...
bridge: frame03 -> frame04
shot2: frame04 -> frame05 -> frame06
```

### 12.4 如果 frame03 与 frame04 差异过大
...
这是 v1.4 的关键新增内容。由于不同团队的后期能力不同，工作流必须明确支持两种模式。
...
### 14.1 选择 Direct Concatenation Mode 的条件

若满足以下条件，优先 Direct：

1. 系统默认将每个 shot 的整段 mp4 直接拼接；
2. 没有后期裁剪模块；
3. 希望完全自动化；
4. 当前 case 更强调稳健和可批量运行。

### 14.2 选择 Trim-and-Overlap Mode 的条件
...
2. 可接受 shot 只保留部分片段；
3. 当前 case 对剪辑自然度要求高；
4. 存在复杂跨-shot动作，需要利用 handle 选取最佳衔接点。

### 14.3 建议的代码层开关

```python
stitch_mode = "direct"  # or "trim_overlap"
```
...
5. 是否考虑用户目标总时长？
6. 是否区分了连续动作和镜头切换？
7. 是否判断了当前 case 是否需要 bridge shot？
8. 是否明确 stitch_mode 是 direct 还是 trim_overlap？
9. 若是 direct，边界帧是否靠近 shot 边界？
10. 若是 trim_overlap，是否明确后期会裁剪 handle？
11. prompt 是否英文、单段、4 到 8 句、现在时？
12. prompt 是否包含 shot、scene、character、action、camera、audio？
13. bridge prompt 是否写清楚转场载体？
14. negative_prompt 是否包含静态画面抑制项和随机转场抑制项？
15. 多参考帧 shot 内部是否写清楚转场细节，而不是让模型随机转场？
16. 各 shot 的拼接点是否自然？
17. 是否给出最终参数代码块？
```
...
```text
v1.4 upgrades the LTX workflow by adding explicit bridge-shot planning and two cross-shot stitching modes: Direct Concatenation for full automatic end-to-end assembly, and Trim-and-Overlap for editorial handle-based stitching. The choice of boundary anchor positions must follow the selected stitching mode.
```

中文概括：

```text
v1.4 将 LTX 工作流进一步升级：正式加入跨-shot桥接段设计，并明确区分“直接拼接模式”和“裁剪余量模式”两套规则。边界参考帧的位置不再一刀切，而必须服从当前选择的拼接模式。
```
